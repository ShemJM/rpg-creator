...
func _build_graphic_picker() -> void:
	var extra := get_node_or_null("PageConditionsExtra")
	if extra == null:
		return
	extra.add_child(VSeparator.new())
	var lbl := Label.new()
	lbl.text = "Graphic:"
	extra.add_child(lbl)
	_graphic_btn = Button.new()
	_graphic_btn.text = "(none)"
	_graphic_btn.custom_minimum_size = Vector2(120, 0)
	_graphic_btn.pressed.connect(_on_edit_graphic)
	extra.add_child(_graphic_btn)


func _update_graphic_button(page: EventPage) -> void:
	if _graphic_btn == null:
		return
	if page.graphic and page.graphic.is_valid():
		_graphic_btn.text = page.graphic.source_path.get_file()
	else:
		_graphic_btn.text = "(none)"


func _on_edit_graphic() -> void:
	if _event == null or _current_page_index >= _event.pages.size():
		return
	var page: EventPage = _event.pages[_current_page_index]
	if page.graphic == null:
		page.graphic = CharacterGraphic.new()
	_open_graphic_edit_popup(page)


func _open_graphic_edit_popup(page: EventPage) -> void:
	var g: CharacterGraphic = page.graphic
	var popup := Window.new()
	popup.title = "Event Graphic"
	...
	_add_graphic_spin(vbox, "Frame width (px):", 1, 1024, g.frame_width,
		func(v: float) -> void:
			g.frame_width = int(v)
			refresh_preview.call()
	)
	_add_graphic_spin(vbox, "Frame height (px):", 1, 1024, g.frame_height,
		func(v: float) -> void:
			g.frame_height = int(v)
			refresh_preview.call()
	)
	_add_graphic_spin(vbox, "Columns (frames/dir):", 1, 32, g.columns,
		func(v: float) -> void: g.columns = int(v))
	_add_graphic_spin(vbox, "Rows (directions):", 1, 32, g.rows,
		func(v: float) -> void: g.rows = int(v))
	_add_graphic_spin(vbox, "Walk frames per dir:", 1, 32, g.frames_per_direction,
		func(v: float) -> void: g.frames_per_direction = int(v))
	_add_graphic_spin(vbox, "Anim FPS:", 1, 60, int(g.anim_fps),
...
func _add_graphic_spin(container: VBoxContainer, text: String, min_v: int, max_v: int, value: int, cb: Callable) -> void:
	var row := HBoxContainer.new()
	var label := Label.new()
	label.text = text
